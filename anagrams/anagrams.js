...
all the anagrams or an empty array if there are none. For example:
...
function anagrams(word, words) {
  const wordSplitted = word.split('').sort().join('')
  const wordsSplitted = words.map((w) => w.split('').sort().join(''))

  return words.filter((w, index) => {
    if (wordsSplitted[index] === wordSplitted) {
      return w
    }
  })
}
